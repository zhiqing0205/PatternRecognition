...
需要四个条件：先验概率$P(\omega_i)$、类条件概率密度$p(\mathbf{x}|\omega_i)$、损失函数$\lambda_{ij}$、决策规则。

### 先验极不平衡时的效果

当$P(\omega_1) = 0.99, P(\omega_2) = 0.01$时，决策规则为：
$$\frac{p(\mathbf{x}|\omega_1)}{p(\mathbf{x}|\omega_2)} > \frac{0.01}{0.99} \approx 0.01$$

**仍然生效**，但会严重偏向主导类$\omega_1$。即使$p(\mathbf{x}|\omega_2)$比$p(\mathbf{x}|\omega_1)$大很多倍，只要似然比小于99，仍选择$\omega_1$。这数学上是最优的，因为错分$\omega_1$的期望损失(0.99)远大于错分$\omega_2$的期望损失(0.01)。

### 克服方法

1. **调整损失函数**：设置$\lambda_{12} = 99, \lambda_{21} = 1$平衡决策阈值
2. **重采样技术**：欠采样主导类或过采样少数类
3. **阈值调整**：通过验证集优化决策阈值
4. **评价指标**：使用F1-score、AUC等替代准确率
